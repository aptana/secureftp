...
public class FTPSConnectionFileManager extends FTPConnectionFileManager implements IFTPSConnectionFileManager {
	...
	public void connect(IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			if (messageLogWriter == null) {
				...
				if (messageLogWriter != null) {
					messageLogWriter.println(StringUtils.format("---------- FTPS {0} ----------", host));
					setMessageLogger(ftpClient, messageLogWriter);
				}
			} else {
				messageLogWriter.println(StringUtils.format("---------- RECONNECTING - FTPS {0} ----------", host));
			}
			...
			while (true) {
						...
						try {
							...
							if (features != null && features.length > 0) {
								...
								for (int i = 0; i < supportedMechanisms.length; ++i) {
									if (featuresList.contains("AUTH " + supportedMechanisms[i])) {
				...
				try {
					...
					if ("331".equals(ftpClient.getLastValidReply().getReplyCode())) { //$NON-NLS-1$
						...
						promptPassword(StringUtils.format("FTPS Authentication for {0}", host), "Password was not accepted. Please specify again.");
						safeQuit();
						continue;
					}
					throw e;
	...
	protected FTPClient createFTPClient() {
		try {
			...
			IdeLog.logError(SecureFTPPlugin.getDefault(), StringUtils.EMPTY, e);
		}
		return null;
	}
